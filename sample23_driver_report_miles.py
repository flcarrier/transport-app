...
def adapt_sql(sql):
    ...
    return (
        sql
        .replace("invoice_no GLOB '[0-9]*'", "invoice_no ~ '^[0-9]+$'")
        .replace("?", "%s")
    )
...
def miles_between_zips(zip_a, zip_b):
...
def total_route_miles(origin_zip, stop_1_zip, stop_2_zip, destination_zip):
    ...
    for start_zip, end_zip in zip(zips, zips[1:]):
        segment = miles_between_zips(start_zip, end_zip)
        if segment is None:
            return None
        total += segment
    return total
...
def format_miles(value):
    if value is None:
        return "" if pgeocode is not None else "Install pgeocode"
    return f"{value:,.1f}"
...
def pg_column_names(cur, table_name):
    cur.execute("""
        SELECT column_name
...
def pg_add_column_if_missing(cur, table_name, column_name, column_type):
    if column_name not in pg_column_names(cur, table_name):
...
def init_db_postgres():
    with closing(db()) as conn:
        ...
        if pg_table_exists(cur, "clients") and not pg_table_exists(cur, "customers"):
        ...
        for column_name, column_type in {
            ...
            pg_add_column_if_missing(cur, "loads", column_name, column_type)

        pg_add_column_if_missing(cur, "trucks", "trailer_no", "TEXT")
        ...
        load_columns = pg_column_names(cur, "loads")
        ...
        for column_name, column_type in {
            ...
            pg_add_column_if_missing(cur, "invoices", column_name, column_type)
        ...
        """)
        load_id_col = cur.fetchone()
        if load_id_col and load_id_col["is_nullable"] == "NO":
            cur.execute("ALTER TABLE invoices ALTER COLUMN load_id DROP NOT NULL")
